...
function nextOrderForParent(mysqli $conn, int $parentId): int
{
    $stmt = $conn->prepare("
        SELECT
            COALESCE(MAX(menu_order), 0) + 10 AS next_ord
        FROM pages
        WHERE parent_id = ?
    ");
    ...
    $next = (int)(
        $stmt->get_result()->fetch_assoc()['next_ord']
        ?? 10
    );
...
$pageTemplatesLocal = [
    ...
    'team' => [
        'title' => 'Výpis týmu',
        'description' => 'Stránka s výpisem členů týmu.',
        'icon' => 'bi bi-people',
        'blocks' => false,
    ],
];
...
// --- dostupné levé navigace ---
$navigationOptions = [];

$navResult = $conn->query("
    SELECT
        id,
        name
    FROM navigation
    ...
    <div class="container-fluid py-4">
        ...
        <div class="row g-4">
            ...
            <?php foreach ($pageTemplatesLocal as $key => $tpl): ?>
                ...
                <div class="col-md-6 col-xl-4">
                    ...
                    <a
                        href="add.php?template=<?= htmlspecialchars($key) ?>"
        ...
        <?php if ($homepageExists): ?>
            ...
            <div class="alert alert-info mt-4">
                ...
                Homepage už je vytvořená,
                proto se v nabídce nezobrazuje.
...
if (
    ...
    <div class="container-fluid py-4">
        ...
        <div class="alert alert-warning">
            Homepage už existuje.
            Není možné vytvořit druhou.
        </div>
...
$isBlockPage = !empty(
    $pageTemplatesLocal[$template]['blocks']
);


// --- defaults ---
$parent_id_default = intval(
    $_GET['parent_id'] ?? 0
);

$prefill_order = nextOrderForParent(
    $conn,
    $parent_id_default
);

$sidebar_navigation_id = (int)(
    $_POST['sidebar_navigation_id'] ?? 0
);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $show_breadcrumbs = isset(
        $_POST['show_breadcrumbs']
    )
        ? 1
        : 0;
    ...
    /*
     * Pokud byla zvolena navigace,
     * ověříme, že skutečně existuje a je aktivní.
     */
    if ($sidebar_navigation_id > 0) {

        $navCheck = $conn->prepare("
            SELECT id
            FROM navigation
            WHERE id = ?
              AND is_active = 1
    ...
    if ($title === '') {
        ...
        if (
            $menu_order === ''
            || !is_numeric($menu_order)
        ) {

            $menu_order = nextOrderForParent(
                $conn,
                $parent_id
            );
...
<div class="container-fluid py-4">
    ...
    <div class="admin-page-header-content mb-4">
        ...
        <div>
            ...
            <p class="text-muted mb-0">
                ...
                <strong>
                    <?= htmlspecialchars(
                        $pageTemplatesLocal[$template]['title']
                    ) ?>
                </strong>
    ...
    <div class="alert <?= $isBlockPage ? 'alert-info' : 'alert-secondary' ?>">
        ...
        <?= htmlspecialchars(
            $pageTemplatesLocal[$template]['description']
        ) ?>

        <?php if ($isBlockPage): ?>

            <div class="mt-2">
                Po uložení budete přesměrováni
                do správy bloků této stránky.
            </div>
    ...
    >
        ...
        <div class="row mb-4">
            ...
            <div class="col-md-8">
                ...
                <div class="form-text">
                    Vybraná navigace se zobrazí
                    v levém sloupci stránky.
                </div>
    ...
    ?.addEventListener(
        ...
        async function () {
            ...
            try {
                ...
                if (
                    j.ok
                    && typeof j.next === 'number'
                ) {
                    document
                        .getElementById('menu_order')
                        .value = j.next;
                }
