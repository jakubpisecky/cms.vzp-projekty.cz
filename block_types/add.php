...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        $stmt = $conn->prepare("SELECT id FROM block_types WHERE type = ? LIMIT 1");
    ...
    if (!$errors) {
        $stmt = $conn->prepare("
            INSERT INTO block_types
                (name, type, description, icon, sort_order, is_active, use_in_pages, use_in_articles)
            VALUES
                (?, ?, ?, ?, ?, ?, ?, ?)
        ");
        $stmt->bind_param("ssssiiii", $name, $type, $description, $icon, $sort_order, $is_active, $use_in_pages, $use_in_articles);
        $stmt->execute();
        $stmt->close();

        logAction("Vytvořen typ bloku '$name'");
...
<div class="container-fluid py-4">
    ...
    <div class="admin-page-header-content mb-4">
        <div>
            ...
            <p class="text-muted mb-0">Nový typ bloku pro homepage a univerzální stránky</p>
        </div>
                    ...
                    <div class="col-lg-12">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text"
                                   ...
                                   class="form-control"
                                   value="<?= e($type) ?>"
                                   placeholder="např. hero"
                                   required>

                            <div class="form-text">
                                Používá se jako název souboru v <code>/templates/blocks/</code>, např. <code>hero.php</code>.
                            </div>
                        </div>
