...
if ($q !== '') {
    $where[] = "(name LIKE ? OR position LIKE ? OR email LIKE ? OR phone LIKE ?)";
    $types .= "ssss";
    $like = "%{$q}%";
    $params[] = $like;
    $params[] = $like;
...
<div class="container-fluid py-4">
    ...
    <div class="admin-page-header-content mb-4">
        <div>
            <h2 class="mb-1">
                Tým <small class="text-muted">(<?= (int)$total ?>)</small>
            </h2>
            <p class="text-muted mb-0">
                Správa týmových pracovníků
            </p>
        </div>
    ...
    <?php if ($created): ?>
        <div class="alert alert-success">Pracovník byl vytvořen.</div>
    <?php endif; ?>

    <?php if ($updated): ?>
        <div class="alert alert-success">Pracovník byl upraven.</div>
    <?php endif; ?>

    <?php if ($deleted): ?>
        <div class="alert alert-success">Pracovník byl smazán.</div>
    <?php endif; ?>
            ...
            <form method="get" class="row g-3">
                <div class="col-lg-7">
                    <input type="text"
                           name="q"
                           class="form-control"
                           placeholder="Hledat podle jména, pozice, e-mailu nebo telefonu..."
                           value="<?= e($q) ?>">
                </div>
                    ...
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td>
                                ...
                                <?php if (!empty($row['phone'])): ?>
                                    <div>
                                        ...
                                        <a href="tel:<?= e(preg_replace('/\s+/', '', $row['phone'])) ?>">
                                            <?= e($row['phone']) ?>
                                        </a>
                                    </div>
                                <?php endif; ?>
                            ...
                            <td class="text-center text-nowrap">
                                ...
                                <a href="move.php?id=<?= (int)$row['id'] ?>&dir=up"
                                    class="btn btn-sm btn-outline-secondary"
                                    title="Nahoru">
                                        ↑
                                    </a>

                                    <a href="move.php?id=<?= (int)$row['id'] ?>&dir=down"
                                    class="btn btn-sm btn-outline-secondary"
                                    title="Dolů">
                                        ↓
                                    </a>
                                ...
                                <a href="delete.php?id=<?= (int)$row['id'] ?>"
                                   ...
                                   onclick="return confirm('Opravdu chcete pracovníka smazat?');"
                                   title="Smazat">
                    ...
                    <?php if ($result->num_rows === 0): ?>
                        <tr>
                            <td colspan="7" class="text-center text-muted p-4">
                                Žádní pracovníci nebyli nalezeni.
                            </td>
                        </tr>
                    <?php endif; ?>
                </tbody>
